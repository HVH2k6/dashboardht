...
export default function TypeForm({ initialValues, onFinish, isEdit = false }: TypeFormProps) {
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      if (translationsArray.length === 0) {
        message.error("Vui lòng nhập tên loại địa điểm cho ít nhất 1 ngôn ngữ!");
        return;
      }
    ...
    children: (
      <div style={{ marginTop: 16 }}>
        <Form.Item
          ...
          label={`Tên loại địa điểm (${lang.name})`}
  ...
  return (
    <Form
      ...
      style={{ maxWidth: 800 }}
    >
      <Form.Item label="Icon (Biểu tượng)">
        <Upload
